...
def _xy_to_axial(x, y, size):
    ...
    Incorporates rounding correction logic.
    """
...
def build_hex_graph(df=None):
    ...
    # 1. Calculate spacing constants
    hex_width = math.sqrt(3) * HEX_SIZE
    row_spacing = 1.5 * HEX_SIZE

    # 2. Dynamically find the loop index spans using min and max bounds
    # A buffer of +/- 1 ensures rows/cols partially overlapping the boundary are caught
    ...
    print(f"Generating grid from Row [{min_row} to {max_row}] and Col [{min_col} to {max_col}]...")
    
    # 3. Iterate through calculated bounding range
    for row in range(min_row, max_row + 1):
        ...
        for col in range(min_col, max_col + 1):
            ...
            # Filter centers to stay within your map dimensions (with a half-hex margin)
...
#     row_spacing = 1.5 * hex_height  # (1.5 * HEX_SIZE)
